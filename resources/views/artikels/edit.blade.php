                ...
                <form action="{{ route('artikels.update', $artikel->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-6">
                        ...
                        <select name="kategori" id="kategori"
                            ...
                            <option value="trend" {{ old('kategori', $artikel->kategori) == 'trend' ? 'selected' : '' }}>Trend</option>
                            <option value="news" {{ old('kategori', $artikel->kategori) == 'news' ? 'selected' : '' }}>Berita</option>
                        </select>
                        @error('kategori')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="deskripsi_singkat" id="deskripsi_singkat" rows="3"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500"
                            placeholder="Masukkan deskripsi singkat artikel (maksimal 150 karakter)" required>{{ old('deskripsi_singkat', $artikel->deskripsi_singkat) }}</textarea>
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="konten" id="konten" rows="10"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500"
                            placeholder="Tulis konten artikel Anda di sini..." required>{{ old('konten', $artikel->konten) }}</textarea>
                    ...
                    <div class="mb-6">
                        ...
                        @if ($artikel->gambar)
                            <div class="mt-4">
                                ...
                                <img src="{{ asset('storage/' . $artikel->gambar) }}" class="w-32 h-32 rounded-lg object-cover shadow-sm" alt="Preview">
                            </div>
                        @endif
                        @error('gambar')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    ...
                    <div class="mb-6">
                        ...
                        <div class="mt-2 space-x-4">
                            <label class="inline-flex items-center">
                                <input type="radio" name="status" value="published" class="form-radio text-indigo-600"
